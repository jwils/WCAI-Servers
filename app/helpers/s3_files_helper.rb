  ...
  def print_directory_structure(project, root)
    ...
    root.children.each do |value|
      if value.is_directory?
        response_string += print_directory_structure(project, value)
      else
          response_string += "<li class=\"page #{value.extension_css} #{"large_file" if value.size > 1024 * 1024 * 1024}\">"
          response_string += link_to value.file_name + " (#{value.str_size})", project_project_file_path(project, CGI::escape(value.path.gsub(" ","%20").gsub("/","%2F"))), :target => '_blank'
          response_string += "</li>"
      end
    end
